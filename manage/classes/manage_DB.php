	...
	while( $campo = mysql_fetch_array($query)){
	 ...
	 function showCodesTableActivated(){
	 	...
	 	$string = "SELECT `Codigo`, `idCodigo`
	 				,`OrderId`
	 				,`ServerInfo`		
	 				,(CASE 1 WHEN datediff(`FechaExpiracion`,current_date()) > 0 THEN datediff(`FechaExpiracion`,current_date()) ELSE 0 END) AS Dias
		 			,`FechaExpiracion` 
	 				,(CASE 1 WHEN `Inactivo` = 1 OR datediff(`FechaExpiracion`,current_date()) < 1 THEN 'Inactive' ELSE 'Active' end) as Status
	 				FROM codes
	 				WHERE 	idClient =".$_SESSION['user_id']." 
	 				  	AND Status = 1 and datediff(`FechaExpiracion`,current_date()) > - 2
